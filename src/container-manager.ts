...
export default class ContainerManager {
  ...
  async inspectConfig(configPath: string): Promise<void> {
    ...
    for (const [name, container] of Object.entries(
      config.containers as { [name: string]: ContainerConfig }
    )) {
      try {
        log.info(` - Validating "${name}"`);

        const dockerContainerInfo = containers.find((c) =>
    ...
    for (const [hostname, container] of hostnames.entries()) {
      try {
        ...
        const network = Object.values(
          dockerContainerInfo.NetworkSettings.Networks
        );
        if (network.length === 0) {
          continue;
        }
