...
async function main(): Promise<void> {
  ...
  if (action === "create") {
    const { name, publicKey } = await inquirer.prompt([
      {
        type: "input",
        name: "name",
    ...
    config.containers[name] = {
      hostnames: [`${name}.nia.404oops.com`],
      ssh_keys: [publicKey],
    };
  } else if (action === "edit") {
    ...
    if (config.containers[container]) {
      ...
      switch (toEdit) {
        ...
        case "remove_hostname":
          const { hostnameToRemove } = await inquirer.prompt([
            {
              type: "list",
          ...
          config.containers[container].hostnames = config.containers[
            container
          ].hostnames.filter(
            (hostname: string) => hostname !== hostnameToRemove
          );
          break;
        case "remove_ssh_key":
          const { sshKeyToRemove } = await inquirer.prompt([
            {
              type: "list",
              name: "sshKeyToRemove",
              message: "Which SSH key do you want to remove?",
              choices: config.containers[container].ssh_keys.map(
                (sshKey: string) =>
                  sshKey.slice(0, 25) +
                  "... " +
                  sshKey.split(" ").slice(2).join(" ")
              ),
            },
          ]);

          config.containers[container].ssh_keys = config.containers[
            container
          ].ssh_keys.filter(
            (sshKey: string) =>
              sshKey.slice(0, 25) +
